...
def run_engine(logs, rules):
    ...
    for entry in logs:
        for rule in rules:
            if entry.get("type") == rule.get("log_type") or rule.get("log_type") == "any":
...
def print_alerts(alerts):
    ...
    for a in alerts:
        print(f"[{a['severity'].upper()}] {a['rule_name']} (ID: {a['rule_id']})")
...
def main():
    parser = argparse.ArgumentParser(description="Run alert rules against parsed log output")
